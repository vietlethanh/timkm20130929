...
class ImageManipulator
{
     ...
     * @throws RuntimeException
     */
    public function resample($filePath,$fileType, $width, $height, $constrainProportions = true)
    {
        if (!is_resource($this->image)) {
		...
		switch ($fileType) {
			case 'image/jpeg':
				imagejpeg($temp, $filePath, 100);
				break;
			case 'image/png':
				imagepng($temp, $filePath, 0);
				break;
			case 'image/gif':
				imagegif($temp, $filePath);
				break;
			default:
				exit;
				break;
